...
# ------- Cluster and FreeIPA instances and networking -------
...
resource "aws_eip" "pvc-base" {
  for_each = { for idx, host in concat(module.bastion.hosts, module.freeipa.hosts) : idx => host }
...
resource "ansible_group" "proxied_servers" {
  ...
  variables = {
    ansible_ssh_common_args = "-o ProxyCommand='ssh -i {{ lookup('ansible.builtin.env', 'SSH_PRIVATE_KEY_FILE') }} -o User=${local.ami_user} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -W %h:%p -q ${aws_eip.pvc-base[0].public_ip}'"
  }
}
...
resource "ansible_host" "freeipa" {
  ...
  # Replace the IP regex if using a non-"magic" DNS resolver
  name = format("%s.%s.%s", each.value.tags["Name"], replace(aws_eip.pvc-base[each.key + length(module.bastion.hosts)].public_ip, ".", "-"), var.public_domain)
  ...
  variables = {
    ansible_host = aws_eip.pvc-base[each.key + length(module.bastion.hosts)].public_ip
    ansible_user = local.ami_user
  }
}
